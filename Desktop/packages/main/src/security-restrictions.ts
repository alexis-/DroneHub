...
const CONTEXT = 'SecurityRestrictions';
...
app.on('web-contents-created', (_, contents) => {
  ...
  contents.on('will-navigate', (event, url) => {
    ...
    console.warn(CONTEXT, `Blocked navigation to disallowed origin: ${origin}`);
  });
  ...
  contents.session.setPermissionRequestHandler((webContents, permission, callback) => {
    ...
    if (!permissionGranted) {
      console.warn(CONTEXT, `Permission '${permission}' denied for origin: ${origin}`);
    }
  });
  ...
  contents.setWindowOpenHandler(({url}) => {
    ...
    if (ALLOWED_EXTERNAL_ORIGINS.has(origin as `https://${string}`)) {
      ...
      shell.openExternal(url).catch(error => {
        console.error(CONTEXT, `Failed to open external URL: ${url}`, error);
      });
    } else {
      console.warn(CONTEXT, `Blocked opening disallowed origin: ${origin}`);
    }
  ...
  contents.on('will-attach-webview', (event, webPreferences, params) => {
    ...
    if (!ALLOWED_ORIGINS_AND_PERMISSIONS.has(origin)) {
      console.warn(CONTEXT, `Blocked webview attachment from: ${params.src}`);
      event.preventDefault();
      return;
    }
